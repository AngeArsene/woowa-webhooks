...
function get_phone_number(array $payload): string
{
    ...
    // Case 2: No + but starts with valid country code
    if (preg_match('/^' . $validCountryPattern . '\d{4,14}$/', $number)) {
        $number = '+' . $number;
        if (preg_match($regex, $number)) return $number;
    }

    // Case 3: Default to Cameroon (+237)
